...
interface LiveStats {
  plans: number; activePlans: number; platforms: number; modules: number; planModules: number;
}

const REPORT_ITEMS: ReportItem[] = [
  {
    id: 'P-001', category: 'Data Integrity', title: 'Zero or negative plan prices allowed without validation',
    severity: 'critical',
    description: 'The plan creation form accepts any numeric value for price fields (monthly_price, yearly_price). Zero or negative prices can be saved without error. This can result in subscriptions being created at ₹0 or negative amounts.',
    location: 'src/app/plans/page.tsx — plan form, price fields; no min value validation',
    recommendation: 'Add client-side validation: price must be > 0. Show an inline error if price ≤ 0. Also add a DB CHECK constraint: CHECK (monthly_price >= 0 AND yearly_price >= 0).',
  },
  {
    id: 'P-002', category: 'Feature Gap', title: 'No subscriber count shown on plan cards',
    severity: 'warning',
    description: 'Plan cards show name, price, and modules but do not show how many active subscriptions are using each plan. This makes it impossible to assess plan popularity or safely delete a plan.',
    location: 'src/app/plans/page.tsx — plan card rendering, no subscription count',
    recommendation: 'Add a "X subscribers" badge to each plan card by querying COUNT(*) from subscriptions WHERE plan_id = plan.id AND status = "active".',
  },
  {
    id: 'P-003', category: 'Data Integrity', title: 'No protection against deleting plans with active subscriptions',
    severity: 'warning',
    description: 'The plan delete button immediately deletes the plan without checking if any active subscriptions reference it. Deleting a plan with active subscribers will orphan those subscriptions (plan_id becomes invalid).',
    location: 'src/app/plans/page.tsx — handleDelete() function, no pre-delete check',
    recommendation: 'Before deleting a plan, query subscriptions WHERE plan_id = planId AND status = "active". If count > 0, show a blocking error: "Cannot delete — X active subscriptions use this plan."',
  },
  {
    id: 'P-004', category: 'Feature Gap', title: 'No side-by-side plan comparison view',
    severity: 'info',
    description: 'The plans page shows individual plan cards but there is no way to compare plans side-by-side (features, pricing, module differences). Sales teams need this to explain plan differences to prospects.',
    location: 'src/app/plans/page.tsx — no comparison view',
    recommendation: 'Add a "Compare Plans" button that opens a comparison table showing all plans as columns with modules as rows, with checkmarks for included modules.',
  },
];
...
export default function PlansReportPage() {
  ...
  const runChecks = async () => {
    ...
    const tables = ['saas_plans', 'saas_platforms', 'saas_modules', 'plan_modules', 'subscriptions'];
    ...
    try {
      const activeR = await supabase.from('saas_plans').select('*', { count: 'exact', head: true }).eq('is_active', true);
      ...
      const planModulesRow = results.find(r => r.table === 'plan_modules');
      setLiveStats({
        plans: plansRow?.rowCount ?? 0, activePlans: activeR.count ?? 0,
        platforms: platformsRow?.rowCount ?? 0, modules: modulesRow?.rowCount ?? 0,
        planModules: planModulesRow?.rowCount ?? 0,
      });
  ...
  return (
    <div className="min-h-screen px-4 py-6 lg:px-8 max-w-screen-2xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><FileText size={20} style={{ color: '#60a5fa' }} />Plans Module — Full Analysis &amp; Debug Report</h1>
      ...
      {liveStats && (
        <CollapsibleSection title="Live Module Statistics" icon={BarChart2} color="#60a5fa" defaultOpen={true}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
            {[
              { label: 'Total Plans', value: liveStats.plans, color: '#3b82f6' },
              { label: 'Active Plans', value: liveStats.activePlans, color: '#34d399' },
              { label: 'Platforms', value: liveStats.platforms, color: '#a78bfa' },
              { label: 'Modules', value: liveStats.modules, color: '#fbbf24' },
              { label: 'Plan-Module Links', value: liveStats.planModules, color: '#fb923c' },
            ].map((s) => (
      ...
      <CollapsibleSection title="Module Functionality Overview" icon={CheckCircle} color="#34d399" defaultOpen={true}>
        <div className="space-y-2 mt-3">
          {[
            { feature: 'Plan CRUD (Create, Edit, Delete)', status: 'ok' as const, note: 'Full CRUD on saas_plans table' },
            { feature: 'Module assignment per plan', status: 'ok' as const, note: 'Checkbox-based module selection stored in plan_modules' },
            { feature: 'Plan search', status: 'ok' as const, note: 'Client-side search by plan name' },
            { feature: 'Active/inactive toggle', status: 'ok' as const, note: 'Toggle plan is_active flag' },
            { feature: 'Price validation (> 0)', status: 'fail' as const, note: 'Zero/negative prices allowed (P-001)' },
            { feature: 'Subscriber count per plan', status: 'fail' as const, note: 'No subscription count shown on plan cards (P-002)' },
            { feature: 'Delete protection for active subscriptions', status: 'fail' as const, note: 'No check for active subscriptions before plan delete (P-003)' },
            { feature: 'Plan comparison matrix', status: 'fail' as const, note: 'No side-by-side plan comparison view (P-004)' },
          ].map((item, i) => {
      ...
      <div className="rounded-xl px-5 py-4 text-center" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <p className="text-[12px]" style={{ color: 'rgba(148,163,184,0.5)' }}>
          ClientFlow Plans Module Report &nbsp;·&nbsp; {generatedAt || 'Loading...'} &nbsp;·&nbsp; {REPORT_ITEMS.length} total findings &nbsp;·&nbsp; Auto-refreshes on every page load
        </p>
      </div>
    </div>
  );
