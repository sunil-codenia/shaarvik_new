...
interface LiveStats {
  total: number; todo: number; inProgress: number; done: number; overdue: number; projects: number;
}

const REPORT_ITEMS: ReportItem[] = [
  {
    id: 'T-001', category: 'Feature Gap', title: 'No task assignment to staff members',
    severity: 'critical',
    description: 'Tasks can be created with a title, description, priority, and due date but cannot be assigned to a specific staff member. The tasks table has no assignee_id or assigned_to column. There is no way to track who is responsible for each task.',
    location: 'src/app/tasks/page.tsx — task form, no assignee field; supabase/migrations/20260404090000_projects_and_task_hierarchy.sql',
    recommendation: 'Add an assignee_id FK column to the tasks table referencing user_profiles. Add an "Assign To" dropdown in the task form populated from staff/user_profiles. Show assignee avatar on task cards.',
  },
  {
    id: 'T-002', category: 'Feature Gap', title: 'No task comments or activity thread',
    severity: 'warning',
    description: 'Tasks have no comment/discussion thread. Team members cannot leave notes, updates, or questions on a task. The activities table exists but is not linked to tasks.',
    location: 'src/app/tasks/page.tsx — task detail view, no comments section',
    recommendation: 'Add a task_comments table (id, task_id, user_id, content, created_at) or reuse the activities table with entity_type="task". Show a comment thread on the task detail page.',
  },
  {
    id: 'T-003', category: 'UX Issue', title: 'No overdue task highlighting or notification',
    severity: 'warning',
    description: 'Tasks with a due_date in the past and status not "done" are overdue but are not visually highlighted. The dashboard has an OverdueTasksPanel but the tasks list page itself shows no overdue indicator.',
    location: 'src/app/tasks/page.tsx — task row rendering; src/app/dashboard/components/OverdueTasksPanel.tsx',
    recommendation: 'Add a red "Overdue" badge to task rows where due_date < today AND status != "done". Add an "Overdue" filter tab to the tasks list.',
  },
  {
    id: 'T-004', category: 'Feature Gap', title: 'No task time tracking',
    severity: 'info',
    description: 'There is no way to log time spent on a task. For project billing and productivity tracking, teams need to record hours worked per task.',
    location: 'src/app/tasks/page.tsx — no time tracking UI',
    recommendation: 'Add a time_entries table (id, task_id, user_id, hours, logged_at, notes). Add a "Log Time" button on task detail. Show total logged hours on task cards.',
  },
  {
    id: 'T-005', category: 'Performance', title: 'Tasks loaded without server-side pagination',
    severity: 'info',
    description: 'All tasks are loaded in a single query with no LIMIT. For teams with hundreds of tasks, this causes slow initial load.',
    location: 'src/app/tasks/page.tsx — fetchTasks(), no .range() or .limit()',
    recommendation: 'Implement server-side pagination using Supabase .range(from, to). Add page navigation controls to the tasks list.',
  },
];
...
export default function TasksReportPage() {
  ...
  const runChecks = async () => {
    ...
    const tables = ['tasks', 'projects', 'activities', 'user_profiles'];
    ...
    setDbChecks(results);
    try {
      const [todoR, inProgressR, doneR, projectsR] = await Promise.all([
      ...
      const overdueR = await supabase.from('tasks').select('*', { count: 'exact', head: true }).lt('due_date', today).neq('status', 'done');
      const totalRow = results.find(r => r.table === 'tasks');
      setLiveStats({ total: totalRow?.rowCount ?? 0, todo: todoR.count ?? 0, inProgress: inProgressR.count ?? 0, done: doneR.count ?? 0, overdue: overdueR.count ?? 0, projects: projectsR.count ?? 0 });
  ...
  return (
    <div className="min-h-screen px-4 py-6 lg:px-8 max-w-screen-2xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><FileText size={20} style={{ color: '#60a5fa' }} />Tasks Module — Full Analysis &amp; Debug Report</h1>
      ...
      {liveStats && (
        <CollapsibleSection title="Live Module Statistics" icon={BarChart2} color="#60a5fa" defaultOpen={true}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
            {[
              { label: 'Total Tasks', value: liveStats.total, color: '#3b82f6' },
              { label: 'To Do', value: liveStats.todo, color: '#94a3b8' },
              { label: 'In Progress', value: liveStats.inProgress, color: '#fbbf24' },
              { label: 'Done', value: liveStats.done, color: '#34d399' },
              { label: 'Overdue', value: liveStats.overdue, color: '#f87171' },
              { label: 'Projects', value: liveStats.projects, color: '#a78bfa' },
            ].map((s) => (
      ...
      <CollapsibleSection title="Module Functionality Overview" icon={CheckCircle} color="#34d399" defaultOpen={true}>
        <div className="space-y-2 mt-3">
          {[
            { feature: 'Task List with Status Filter', status: 'ok' as const, note: 'Loads from Supabase with status and priority filters' },
            { feature: 'Add Task (/tasks/add)', status: 'ok' as const, note: 'Full form with title, description, priority, due date, project' },
            { feature: 'Edit Task (/tasks/[id]/edit)', status: 'ok' as const, note: 'Updates tasks table via Supabase' },
            { feature: 'Delete Task', status: 'ok' as const, note: 'Hard delete with confirmation' },
            { feature: 'Task Detail View (/tasks/[id])', status: 'ok' as const, note: 'Shows task info, priority, status, due date' },
            { feature: 'Task Assignment to Staff', status: 'fail' as const, note: 'No assignee_id column — tasks cannot be assigned (T-001)' },
            { feature: 'Task Comments / Activity Thread', status: 'fail' as const, note: 'No comment thread on tasks (T-002)' },
            { feature: 'Overdue Task Highlighting', status: 'fail' as const, note: 'No overdue badge on task list rows (T-003)' },
            { feature: 'Time Tracking', status: 'fail' as const, note: 'No time logging on tasks (T-004)' },
            { feature: 'Server-side Pagination', status: 'warn' as const, note: 'All tasks loaded in one query — no DB-level pagination (T-005)' },
          ].map((item, i) => {
      ...
      <div className="rounded-xl px-5 py-4 text-center" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <p className="text-[12px]" style={{ color: 'rgba(148,163,184,0.5)' }}>
          ClientFlow Tasks Module Report &nbsp;·&nbsp; {generatedAt || 'Loading...'} &nbsp;·&nbsp; {REPORT_ITEMS.length} total findings &nbsp;·&nbsp; Auto-refreshes on every page load
        </p>
      </div>
    </div>
  );
