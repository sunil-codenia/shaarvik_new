...
interface LiveStats {
  total: number; open: number; inProgress: number; resolved: number; closed: number; clients: number;
}

const REPORT_ITEMS: ReportItem[] = [
  {
    id: 'SP-001', category: 'Feature Gap', title: 'No SLA (Service Level Agreement) tracking',
    severity: 'critical',
    description: 'Support tickets have no SLA configuration. There is no way to define response time targets (e.g., P1 = 1 hour, P2 = 4 hours) or track whether tickets are being resolved within SLA. No SLA breach alerts exist.',
    location: 'src/app/tickets/page.tsx — ticket form, no SLA fields; supabase/migrations/20260403290000_support_tickets.sql',
    recommendation: 'Add SLA fields to tickets: sla_due_at (computed from priority + created_at), sla_breached (boolean). Create a priority-to-SLA mapping table. Show a countdown timer on open tickets and highlight breached SLAs in red.',
  },
  {
    id: 'SP-002', category: 'Feature Gap', title: 'No ticket assignment to staff members',
    severity: 'warning',
    description: 'Tickets can be created and viewed but cannot be assigned to a specific support agent. There is no assigned_to field on tickets. All tickets appear in a shared queue with no ownership.',
    location: 'src/app/tickets/page.tsx — ticket form, no assignee field',
    recommendation: 'Add assigned_to FK column to tickets referencing user_profiles. Add an "Assign To" dropdown in the ticket detail view. Show assignee name/avatar on ticket list rows.',
  },
  {
    id: 'SP-003', category: 'Feature Gap', title: 'No internal notes / private comments on tickets',
    severity: 'warning',
    description: 'Ticket replies are visible to all. There is no way to add internal notes (visible only to staff, not the client). Internal notes are essential for support teams to coordinate without exposing internal discussions.',
    location: 'src/app/tickets/[id]/page.tsx — reply section, no internal note toggle',
    recommendation: 'Add an is_internal boolean to ticket replies/comments. Show internal notes with a distinct background (e.g., yellow) and a "Staff Only" badge. Filter out internal notes when generating client-facing ticket views.',
  },
  {
    id: 'SP-004', category: 'UX Issue', title: 'No ticket priority escalation workflow',
    severity: 'info',
    description: 'Ticket priority (low/medium/high/urgent) can be set manually but there is no automatic escalation. A ticket that has been open for 48 hours without response should auto-escalate to a higher priority.',
    location: 'src/app/tickets/page.tsx — priority field, no escalation logic',
    recommendation: 'Add a Supabase scheduled function that checks open tickets older than X hours and upgrades their priority. Send a notification to the assigned agent and supervisor on escalation.',
  },
  {
    id: 'SP-005', category: 'Feature Gap', title: 'No customer satisfaction (CSAT) rating after ticket resolution',
    severity: 'info',
    description: 'When a ticket is resolved, there is no mechanism to collect customer satisfaction feedback. CSAT scores are essential for measuring support quality.',
    location: 'src/app/tickets/[id]/page.tsx — no CSAT section after resolution',
    recommendation: 'When a ticket status changes to "resolved", trigger an email to the client with a 1-5 star rating link. Store the rating in a ticket_ratings table. Display average CSAT on the support dashboard.',
  },
];
...
export default function TicketsReportPage() {
  ...
  const runChecks = async () => {
    ...
    const tables = ['support_tickets', 'clients', 'user_profiles'];
    ...
    setDbChecks(results);
    try {
      const [openR, inProgressR, resolvedR, closedR, clientsR] = await Promise.all([
      ...
      const totalRow = results.find(r => r.table === 'support_tickets');
      setLiveStats({ total: totalRow?.rowCount ?? 0, open: openR.count ?? 0, inProgress: inProgressR.count ?? 0, resolved: resolvedR.count ?? 0, closed: closedR.count ?? 0, clients: clientsR.count ?? 0 });
  ...
  return (
    <div className="min-h-screen px-4 py-6 lg:px-8 max-w-screen-2xl mx-auto space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-white flex items-center gap-2"><FileText size={20} style={{ color: '#60a5fa' }} />Support Module — Full Analysis &amp; Debug Report</h1>
      ...
      {liveStats && (
        <CollapsibleSection title="Live Module Statistics" icon={BarChart2} color="#60a5fa" defaultOpen={true}>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
            {[
              { label: 'Total Tickets', value: liveStats.total, color: '#3b82f6' },
              { label: 'Open', value: liveStats.open, color: '#f87171' },
              { label: 'In Progress', value: liveStats.inProgress, color: '#fbbf24' },
              { label: 'Resolved', value: liveStats.resolved, color: '#34d399' },
              { label: 'Closed', value: liveStats.closed, color: '#94a3b8' },
              { label: 'Total Clients', value: liveStats.clients, color: '#a78bfa' },
            ].map((s) => (
      ...
      <CollapsibleSection title="Module Functionality Overview" icon={CheckCircle} color="#34d399" defaultOpen={true}>
        <div className="space-y-2 mt-3">
          {[
            { feature: 'Ticket List with Status Filter', status: 'ok' as const, note: 'Loads from Supabase with status and priority filters' },
            { feature: 'Create New Ticket (/tickets/new)', status: 'ok' as const, note: 'Full form with subject, description, priority, client' },
            { feature: 'Ticket Detail View (/tickets/[id])', status: 'ok' as const, note: 'Shows ticket info, replies, status updates' },
            { feature: 'Reply to Ticket', status: 'ok' as const, note: 'Adds reply to ticket thread' },
            { feature: 'Status Update (open/in_progress/resolved/closed)', status: 'ok' as const, note: 'Status changes tracked in DB' },
            { feature: 'Ticket Assignment to Staff', status: 'fail' as const, note: 'No assigned_to field — tickets cannot be assigned (SP-002)' },
            { feature: 'SLA Tracking', status: 'fail' as const, note: 'No SLA configuration or breach alerts (SP-001)' },
            { feature: 'Internal Notes (Staff Only)', status: 'fail' as const, note: 'No private internal notes on tickets (SP-003)' },
            { feature: 'Priority Escalation', status: 'fail' as const, note: 'No auto-escalation for stale tickets (SP-004)' },
            { feature: 'CSAT Rating after Resolution', status: 'fail' as const, note: 'No customer satisfaction collection (SP-005)' },
          ].map((item, i) => {
      ...
      <div className="rounded-xl px-5 py-4 text-center" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)' }}>
        <p className="text-[12px]" style={{ color: 'rgba(148,163,184,0.5)' }}>
          ClientFlow Support Module Report &nbsp;·&nbsp; {generatedAt || 'Loading...'} &nbsp;·&nbsp; {REPORT_ITEMS.length} total findings &nbsp;·&nbsp; Auto-refreshes on every page load
        </p>
      </div>
    </div>
  );
